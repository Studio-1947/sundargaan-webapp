    ...
    >
      ...
      {type === 'video' && (
        <div className="h-40 overflow-hidden bg-black">
          <video src={blob.url} className="w-full h-full object-contain" controls={false} muted />
        </div>
      )}

      {/* Info */}
      <div className="p-3 space-y-2">
        <p className="text-sm font-medium truncate" style={{ color: '#1a1005' }} title={getFilename(blob.pathname)}>
        ...
        <div className="flex justify-end pt-1">
          {confirmDelete ? (
            ...
            <button
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.backgroundColor = '#F1D8CD'
                e.currentTarget.style.color = '#CB460C'
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.backgroundColor = '#F7EAE5'
...
function DropZone({
  ...
  return (
    <div
      ...
      style={{
        borderColor: dragging ? '#CB460C' : '#e5d5cd',
        backgroundColor: dragging ? '#FFF4F0' : '#FEFCFB',
      }}
    >
      <input
        ref={inputRef}
      ...
      {uploading ? (
        <>
          ...
          />
          <p className="text-sm font-medium" style={{ color: '#CB460C' }}>Uploading…</p>
        </>
      ) : (
        <>
          <div
          ...
          <div className="text-center">
            <p className="text-sm font-medium" style={{ color: '#1a1005' }}>
              Drop {tab.label.toLowerCase()} here or{' '}
              <span style={{ color: '#CB460C' }}>browse</span>
            </p>
            <p className="text-xs mt-1" style={{ color: '#a89080' }}>
              {tab.accept.split(',').map(m => m.split('/')[1]?.toUpperCase()).join(', ')} · Max {tab.maxSizeMb} MB each
            </p>
          </div>
        </>
      )}
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 pointer-events-none">
      <AnimatePresence>
        {toasts.map((t) => (
          ...
          >
            {t.type === 'success' ? (
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#CB460C" strokeWidth="2.5">
                <polyline points="20 6 9 17 4 12" />
              </svg>
            ) : (
...
const AdminUploadsPage: React.FC = () => {
  ...
  const [blobs, setBlobs] = useState<Record<UploadType, BlobItem[]>>({
  ...
  const loadBlobs = useCallback(async (type: UploadType) => {
    setLoading(true)
    try {
      const prefix = TABS.find((t) => t.id === type)!.prefix
      const items = await listBlobs(prefix)
      setBlobs((prev) => ({ ...prev, [type]: items }))
    } catch (e: any) {
  ...
  const handleUpload = async (files: File[]) => {
    ...
    for (const file of files) {
      try {
        const result = await uploadFile(file, activeTab, tab.prefix.replace('/', ''))
        ...
        setBlobs((prev) => ({ ...prev, [activeTab]: [newBlob, ...prev[activeTab]] }))
        successCount++
      } catch (e: any) {
    ...
    if (successCount > 0)
      addToast('success', `${successCount} file${successCount > 1 ? 's' : ''} uploaded successfully`)
  ...
  const handleDelete = async (url: string) => {
    try {
      ...
      setBlobs((prev) => ({
        ...prev,
        [activeTab]: prev[activeTab].filter((b) => b.url !== url),
      }))
  ...
  const currentBlobs = blobs[activeTab]
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: '#F7EAE5' }}>
      ...
      >
        <div className="px-6 py-6 border-b" style={{ borderColor: '#e5d5cd' }}>
          ...
          <div className="mt-4 flex items-center gap-2">
            <span
              className="text-xs font-bold uppercase tracking-[0.2em] px-2 py-0.5 rounded-full"
              style={{ backgroundColor: '#F7EAE5', color: '#CB460C' }}
            >
              Admin
            </span>
          ...
          {TABS.map((t) => {
            ...
            return (
              <button
                ...
                style={{
                  backgroundColor: isActive ? '#F7EAE5' : 'transparent',
                  color: isActive ? '#CB460C' : '#4a3b33',
                }}
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        >
          <div>
            <h1 className="text-xl font-semibold" style={{ color: '#1a1005' }}>
              {tab.label}
            </h1>
            <p className="text-sm mt-0.5" style={{ color: '#a89080' }}>
              {currentBlobs.length} file{currentBlobs.length !== 1 ? 's' : ''} · stored at{' '}
              <span style={{ color: '#CB460C' }}>
                712vsvopahcsqllv.public.blob.vercel-storage.com
              </span>
            </p>
          </div>
          <button
            onClick={() => loadBlobs(activeTab)}
            disabled={loading}
          ...
          >
            <motion.span animate={loading ? { rotate: 360 } : { rotate: 0 }} transition={{ repeat: loading ? Infinity : 0, duration: 1, ease: 'linear' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
        ...
        <div className="flex-1 p-8 space-y-8">
          ...
          {/* Drop zone */}
          <DropZone tab={tab} onUpload={handleUpload} uploading={uploading} />
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ backgroundColor: '#F7EAE5', color: '#e5d5cd' }}>
                {tab.icon}
              </div>
              <p className="text-sm" style={{ color: '#a89080' }}>
                No {tab.label.toLowerCase()} uploaded yet
              </p>
            </div>
          ) : (
            <div>
